...
const Settings = {
  render(container) {
    container.innerHTML = `
      <div class="page-header">
        <div><div class="page-title">Settings</div><div class="page-sub">Server connection and plugin configuration.</div></div>
        <div class="page-actions">
          <button class="btn btn-primary btn-sm" onclick="Settings.save()">💾 Save Settings</button>
        </div>
      </div>
      <div class="settings-grid">
          ...
          <div class="card">
            ...
            <div style="display:flex;flex-direction:column;gap:10px">
              ...
              <div class="field-group">
                ...
                <div style="font-size:10.5px;color:var(--text3);margin-top:3px">* = allow all (dev only). Production: use your panel domain.</div>
              </div>
              <div class="field-group">
                <label class="field-label">Rate Limit (req/min per IP)</label>
                <input class="field-input" type="number" id="cfgRateLimit" value="120"/>
                <div style="font-size:10.5px;color:var(--text3);margin-top:3px">Prevents API abuse. 120 = 2 req/sec, sufficient for any dashboard.</div>
              </div>
            </div>
          </div>
        ...
        <div class="settings-section">
          <div class="card">
            ...
            <div id="serverStatusCard" style="display:flex;flex-direction:column;gap:8px">
              <div class="skeleton" style="height:20px"></div>
              <div class="skeleton" style="height:20px"></div>
              <div class="skeleton" style="height:20px"></div>
            </div>
            <button class="btn btn-ghost btn-sm" style="margin-top:10px" onclick="Settings.refreshStatus()">↻ Refresh Status</button>
          </div>
          ...
          <div class="card">
            ...
            <div style="display:flex;flex-direction:column;gap:8px">
              <button class="btn btn-ghost btn-sm" onclick="Settings.reloadPlugin()">
                ...
                Reload Plugin (all crates)
              </button>
              <button class="btn btn-danger btn-sm" onclick="Settings.logout()">🚪 Logout</button>
            </div>
          </div>
        </div>
      </div>
  ...
  async refreshStatus() {
    ...
    try {
      ...
      card.innerHTML = `
        <div style="display:flex;justify-content:space-between;font-size:12px"><span style="color:var(--text2)">Status</span><span style="color:var(--green);font-weight:600">● Online</span></div>
      ...
      card.innerHTML = `<div style="color:var(--red);font-size:12px">● Offline — ${e.message}</div>`;
